...
// Probe each item's primary src, capped at `perHost` requests in flight to any
// one host (so a single CDN is never hammered) AND `concurrency` in flight
// overall. `delayMs` spaces successive requests to the same host. Many distinct
// hosts → high parallelism; one host → gentle. `probe` is injectable for tests.
async function checkLinks(items, { concurrency = 16, perHost = 4, delayMs = 0, probe = defaultProbe } = {}) {
  const results = new Array(items.length);

  // bucket item indices by host — each host runs as its own bounded stream
  const buckets = new Map();
  items.forEach((it, k) => {
    const h = hostOf(it.src[0]);
    if (!buckets.has(h)) buckets.set(h, []);
    buckets.get(h).push(k);
  });

  // global gate so total in-flight never exceeds `concurrency`
  let inFlight = 0;
  const waiters = [];
  const acquire = () => (inFlight < concurrency ? (inFlight++, Promise.resolve()) : new Promise((r) => waiters.push(r)));
  const release = () => { inFlight--; if (waiters.length) { inFlight++; waiters.shift()(); } };

  async function runHost(indices) {
    let p = 0;
    async function hostWorker() {
      while (p < indices.length) {
        ...
        await acquire();
        try {
          const r = await probe(items[k].src[0]);
          results[k] = { url: items[k].src[0], item: items[k], ...r };
        } finally { release(); }
        if (delayMs) await new Promise((r) => setTimeout(r, delayMs));
      }
    }
    // at most `perHost` concurrent requests to this host
    await Promise.all(Array.from({ length: Math.min(Math.max(1, perHost), indices.length) }, hostWorker));
  }

  await Promise.all([...buckets.values()].map(runHost));
  return results;
}
...
async function runImport(argv) {
  ...
  const perHost = Number(flag('--per-host', 4));
  ...
  if (doCheck) {
    console.log(`\nHEAD-checking ${manifest.items.length} url(s) (max ${perHost}/host, ${concurrency} total${delayMs ? `, ${delayMs}ms/host` : ''})…`);
    const results = await checkLinks(manifest.items, { concurrency, perHost, delayMs });
...
module.exports = { libraryToManifest, checkLinks, complete, resolveRel, hostOf, runImport };
