...
const { adoptLocal } = require('./adopt');
...
async function pack({ all = false, adopt = true } = {}) {
  if (!age.haveRecipient()) {
    throw new Error('.age-recipient missing — run scripts/init.sh once to create the key.');
  }
  // Register any hand-dropped files first, so `pack` on freshly-dropped media
  // just works (folder under media/ = album). Pass adopt:false to skip.
  if (adopt) adoptLocal();
  require('./dims').backfillDims();     // record w/h for anything still missing it
